        ...
        if creds and creds.expired and creds.refresh_token:
            ...
            if os.path.exists('credentials.json'):
                ...
                if creds_path and os.path.exists(creds_path):
                    ...
                    raise Exception("No valid credentials found. Please provide credentials.json or set GOOGLE_APPLICATION_CREDENTIALS environment variable.")
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent | types.ImageContent]:
    ...
    if name == "list_files":
        ...
        file_list = "\n".join(file_list_items)
        return [types.TextContent(type="text", text=f"Search results for '{query_text}' ({len(files)}):\n\n{file_list}")]
        ...
        try:
            ...
            # For text files
            if file['mimeType'] == 'text/plain' or file['mimeType'].startswith('text/') or file['mimeType'] == 'application/json':
                ...
                return [types.TextContent(type="text", text=f"Content of '{file['name']}':\n\n{content}")]
            ...
            # For Google Docs
                ...
                content = fh.getvalue().decode('utf-8')
                return [types.TextContent(type="text", text=f"Content of Google Doc '{file['name']}':\n\n{content}")]
            ...
            # For Google Sheets
                ...
                content = fh.getvalue().decode('utf-8')
                return [types.TextContent(type="text", text=f"Content of Google Sheet '{file['name']}' (CSV format):\n\n{content}")]
        ...
        try:
            ...
            return [types.TextContent(type="text", text=f"Error retrieving file metadata: {str(e)}")]
        ...
        if not folders:
            parent_name = "Root" if parent_id == "root" else service.files().get(fileId=parent_id, fields="name").execute()['name']
        ...
        return [types.TextContent(type="text", text=f"Folders in '{parent_name}' ({len(folders)}):\n\n{folder_list}")]
